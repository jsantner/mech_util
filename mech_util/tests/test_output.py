...
def test_convert_mech_irrev():
    """
    """
    mech = relative_location(os.path.join('mechanisms', 'mech.dat'))
    therm = relative_location(os.path.join('mechanisms', 'therm.dat'))

    with open(relative_location(os.path.join('mechanisms', 'mech_blessed.txt')), 'r') as f:
        blessed = f.read()
